...
<div
	class="flex h-full w-24 flex-none flex-col items-center justify-around py-3 lg:w-28 lg:py-6 2xl:w-40 fhd:w-72 wqhd:w-80"
>
	<div class="size-16 lg:size-28 2xl:size-40 fhd:size-52 wqhd:size-72">
		<Vinyl spinning={!trackPaused}></Vinyl>
	</div>
	<div class="flex flex-row items-stretch justify-evenly">
		<span
			class="ml-1 h-fit w-full border-2 border-gray-500 bg-green-500 px-1 py-[0.125rem] text-xs font-bold text-white lg:text-sm 2xl:text-xl fhd:border-4 fhd:px-4 fhd:py-2 fhd:text-3xl"
		>
			{formatTrackTime(trackCurrentTime)}
		</span>
		<span
			class="h-fit w-full border-2 border-gray-500 bg-green-500 px-1 py-[0.125rem] text-xs font-bold text-white lg:text-sm 2xl:text-xl fhd:border-4 fhd:px-4 fhd:py-2 fhd:text-3xl"
		>
	...
	<div>
		<MidiBoardInformation bind:time bind:repeats />
	</div>
